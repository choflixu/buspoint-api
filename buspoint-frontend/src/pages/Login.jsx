...
export default function Login() {
  ...
  const handleGoogle = async () => {
    ...
    try {
      ...
      try {
        const res = await loginBackend(result.user.email, 'google-oauth-' + result.user.uid)
        setJwtToken(res.data.token)
      } catch (e) {
        console.warn('Backend login failed:', e.response?.data || e.message)
      }
      navigate('/routes')
    } catch (err) {
      setError(err.message.replace('Firebase: ', ''))
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
        ...
        <div className="flex bg-gray-100 rounded-xl p-1 mb-6">
          <button
            ...
            className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${!isRegister ? 'bg-white shadow text-red-600' : 'text-gray-500'}`}
          >
            Login
          </button>
          <button
